...
BMP* image_convertir_nb(const BMP* image_o) {  // test passed july 17
	UCHAR red, green, blue;
	int nb_col, nb_lig, codage;
	if (image_o == NULL) {
		printf("BMP* IMage is NULL\n");
		return NULL;
	}
	nb_lig = BMP_GetHeight(image_o);
	nb_col = BMP_GetWidth(image_o);
	codage = BMP_GetDepth(image_o);
	printf("BMP picture siz= %d,%d,%d\n", nb_col, nb_lig, codage);
	BMP* nouvelle;
	...
	if (nouvelle == NULL) {
		printf("memory alloc errer in BMP_Create,BMP file source maybe wrong! \n");
		return NULL;
	}
	for (pos_x = 0; pos_x < colonnes; ++pos_x) {
		for (pos_y = 0; pos_y < lignes; ++pos_y) {
			UCHAR gris;
			BMP_GetPixelRGB(image_o, col_depart + pos_x, lig_depart + pos_y, &red, &green, &blue);
			gris = 0.3 * red + 0.58 * green + 0.12 * blue;
			if (gris >= COL_DEPT) BMP_SetPixelRGB(nouvelle, pos_x, pos_y, 255, 255, 255);
			else BMP_SetPixelRGB(nouvelle, pos_x, pos_y, 0, 0, 0);
		}
	}
...
void image_remplir_rectangle(BMP* image_o, int x1, int y1, int x2, int y2) {// // test passed july 17
	int  pos_x, pos_y;
	int col_depart = x1;
	int lig_depart = y1;
	for (pos_x = x1; pos_x < x2; ++pos_x) {
		for (pos_y = y1; pos_y < y2; ++pos_y) {
			BMP_SetPixelRGB(image_o, pos_x, pos_y, 0, 0, 0);
		}
	}
}
